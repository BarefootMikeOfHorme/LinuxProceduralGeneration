...
# textual is an optional runtime capability: this module must import without it
# and only expose/launch the TUI when textual is actually installed.
try:
...
TEXTUAL_REQUIREMENT_HINT = (
    "textual is required for the VaultMind Forge TUI "
    "(vaultmind_forge.cli.tui_app). Install with: pip install textual"
)

# TUI widget definitions subclass textual classes, so they are only built when
...
def run() -> None:
    """Launch the TUI. Raises a clear error when textual is not installed."""
    if not TEXTUAL_AVAILABLE:
        raise ImportError(TEXTUAL_REQUIREMENT_HINT)
    VaultMindApp().run()


def __getattr__(name: str):
    """Report the missing optional dependency for TUI entry points."""
    if not TEXTUAL_AVAILABLE and name in ("VaultMindApp", "SystemMonitor", "AgentStatus"):
        raise ImportError(TEXTUAL_REQUIREMENT_HINT)
